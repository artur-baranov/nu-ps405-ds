...
To get a better idea what's going on, you can plot these three independent variables. But what if there are more?
...
It gets harder to interpret the results. Let's write out the model. Then, by plugging the coefficients we can calculate the predicted value for any GDP value, regime type and presence or absence of international conflict.

$$
\text{Import} = \beta_0 + \beta_1(\text{Log_GDP}) + \beta_2(\text{Regime}) + \beta_3(\text{International conflict})
$$
...
Report the results, simulations and marginal effects
